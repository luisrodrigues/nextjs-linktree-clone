...
function TwitterIcon({ href }: { href: string }) {
  return (
  ...
  return (
    ...
    >
      <div className="flex text-center w-full">
        ...
        <h2 className="flex justify-center items-center font-semiboldb w-full text-gray-700 -ml-10">
          {title}
        </h2>
      </div>
    </a>
      ...
      <div className="flex items-center gap-4 mt-6 text-white">
        {data.socials.map((link) => {
          if (link.href.includes("twitter")) {
            return <TwitterIcon {...link} />;
          }
          if (link.href.includes("github")) {
            return <GitHubIcon {...link} />;
          }
        })}
      </div>
    </div>
